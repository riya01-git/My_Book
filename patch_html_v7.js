...
if (scriptMatch) {
  ...
  // We will define PAGES as an empty array initially
  let PAGES = [];
  `;
  
  // Inject the custom pages logic right after script tag opens
  scriptBody = customPagesCode + scriptBody.replace(oldPagesRegex, '');

  // Now we need to modify boot() to populate PAGES asynchronously AFTER fonts load
  ...
  const bootReplacement = `(async function boot(){
    ...
    // Generate pages without blocking the main thread
    for (let i = 0; i < customChapters.length; i++) {
      const chapter = customChapters[i];
      PAGES.push({
        title: chapter.title || '',
    ...
    // Re-initialize M and LAND since PAGES is now populated
    const M = PAGES.length, LAND = 0;
    
    // Re-populate the TOC list now that PAGES exist
    const plateList = document.getElementById('plateList');
    plateList.innerHTML = ''; // clear
    PAGES.forEach((p,i)=>{
