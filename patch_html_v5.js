...
// 3. Fix TOC to skip hidden chapters
const tocOriginal = /PAGES\.forEach\(\(p,i\)=>\{\s*const li=document\.createElement\('li'\);/;
html = html.replace(tocOriginal, "PAGES.forEach((p,i)=>{ if(p.hideFromToc) return; const li=document.createElement('li');");
...
console.log('HTML successfully patched for cover, animation, and TOC!');
