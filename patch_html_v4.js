...
// We need to replace the script body with the updated canvas logic.
// First extract the script body.
const scriptMatch = html.match(/<script>([\s\S]*?)<\/script>/);
if (scriptMatch) {
  let scriptBody = scriptMatch[1];
  
  // Only replace if not already wrapped (since we redownloaded, it shouldn't be wrapped)
  ...
  function generatePageImage(chapter) {
    ...
    gradient.addColorStop(0.5, 'rgba(0,0,0,0.1)'); // spine dark
    ...
    return canvas.toDataURL('image/png'); // Using PNG for transparent corners
  }
  ...
  // Since we re-downloaded the HTML, we just replace the original PAGES logic
...
// Remove empty <a href="#contact"> if there are any left (the earlier regex should have caught top-socials)
html = html.replace(/<a href="#contact"[^>]*>.*?<\/a>/g, '');

fs.writeFileSync(htmlPath, html, 'utf8');
console.log('HTML successfully patched for rounded corners, splits, and text removal!');
